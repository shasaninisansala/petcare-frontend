...
const API_URL = 'http://localhost:8080/api/ai/emergency-chat';
...
export default function EmergencyVetAssistant() {
  ...
  const callAI = async (userMessage) => {
    try {
      ...
      if (data.success && data.text) {
        const aiMessage = { type: 'assistant', text: data.text };
        setMessages(prev => [...prev, aiMessage]);

        // Extract care tips and warnings
        extractCareTips(data.text);
        extractWarnings(data.text);
      } else {
      ...
      const errorMessage = {
        type: 'assistant',
        text: "I'm having trouble connecting. Please try again or contact a veterinarian if this is urgent."
      };
  ...
  const handleSymptomClick = async (symptom) => {
    ...
    await callAI(`My pet is experiencing: ${symptom}`);
  };
